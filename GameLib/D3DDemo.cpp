...
struct TexVertex {
	...
	TexVertex(float x, float y, float z, float nx, float ny, float nz, float u, float v) {
		...
		//_color = D3DCOLOR_RGBA(255,255,255,100);
	}
	float _x, _y, _z;
	float _nx, _ny, _nz;
	//D3DCOLOR _color;
	float _u, _v;
	// remember: must D3DFVF_DIFFUSE then D3DFVF_TEX1
...
bool TexCube_Setup() {
	Device->CreateVertexBuffer(24 * sizeof(TexVertex), D3DUSAGE_WRITEONLY, TexVertex::FVF,
...
bool TexAlpha_Setup() {
	...
	D3DXCreateTextureFromFile(Device, L"cratewalpha.dds", &CrateTex);
	D3DXCreateTextureFromFile(Device, L"lobbyxpos.jpg", &BackDropTex);
...
bool TexAlpha_Display(float timeDelta) {
	D3DXMATRIX xRot;
	...
	Device->SetStreamSource(0, BackDropVB, 0, sizeof(TexVertex));
	Device->SetFVF(TexVertex::FVF);
	...
	int showCmd)
{
	...
	TexAlpha_Setup();
	D3DLib::EnterMsgLoop(TexAlpha_Display);
